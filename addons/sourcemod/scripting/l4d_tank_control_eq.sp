...
public Action Tank_Cmd(int client, int args)
{
    ...
    if (tankClientId != -1)
    {
        ...
        {
            if (client == tankClientId) CPrintToChat(client, "{red}<{default}坦克局{red}> {green}你 {default}将成为{red}坦克{default}!");
            else CPrintToChat(client, "{red}<{default}坦克局{red}> {olive}%s {default}将成为{red}坦克!", tankClientName);
        }
    }
...
{    
    ...
    {
        PrintHintText(tank_index, "第二次控制权开始,消耗较快,请注意控制权");
        for (int i = 1; i <= MaxClients; i++) 
        {
            if (! IsClientInGame(i) || GetClientTeam(i) != 3)
                continue;

            if (tank_index == i) CPrintToChat(i, "{red}<{default}坦克控制权{red}> {olive}二控{red}开始");
            else CPrintToChat(i, "{red}<{default}坦克控制权{red}> {default}({green}%N{default})的{olive}二控{red}开始", tank_index);
        }
...
public void outputTankToAll(any data)
{
    ...
    if (tankClientId != -1)
    {
        ...
        {
            CPrintToChatAll("{red}<{default}坦克局{red}> {olive}%s {default}将成为{red}坦克!", tankClientName);
        }
        else
        {
            for (int i = 1; i <= MaxClients; i++) 
            {
                ...
                if (tankClientId == i) CPrintToChat(i, "{red}<{default}坦克局{red}> {green}你{default}将成为{red}坦克{default}!");
                else CPrintToChat(i, "{red}<{default}坦克局{red}> {olive}%s {default}将成为{red}坦克!", tankClientName);
            }
        }
    }
}
