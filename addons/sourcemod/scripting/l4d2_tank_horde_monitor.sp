...
public void AnnounceTankSpawn()
{
	...
	CPrintToChatAll("<{olive}注意{default}> 当坦克在场时，尸潮将会 {blue}暂停生成{default}! 再次尝试前进 {blue}%0.1f%%{default}路程时，将会重新生成尸潮!", fProgressFlowPercent);
...
public Action FlowCheckTimer(Handle hTimer)
{
	...
	if (fProgressFlowPercent - fWarningPercent >= 1.0){
		...
		CPrintToChatAll("<{red}警告{default}> 再前进 {blue}%0.1f%%{default} 路程时,尸潮将会重新开启……", fWarningPercent);
	}
	...
	if (tankInPlay && IsInfiniteHordeActive()){
		...
		if (announcedHordeMax){
			...
			if (!announcedHordeResume && tankInPlayDelay && fPushAmount >= 0.05){
				...
				CPrintToChatAll("<{red}警告{default}> 尸潮 {blue}已重新启动{default}, 当前强度为 {green}%i%%{default}, 继续推进将会加大尸潮强度", iPushPercent);
			...
			if (fPushAmount - fPushWarningPercent >= 0.20 && fPushAmount != 1.0 && announcedHordeResume){
				fPushWarningPercent = fPushAmount;
				int iPushPercent = RoundToNearest(fPushAmount * 100.0);
				CPrintToChatAll("<{red}警告{default}> 当前尸潮强度为 {green}%i%%{default}...", iPushPercent);
			}

			// Have survivors have pushed past the extra distance we allow?
			if (fPushAmount == 1.0){
				CPrintToChatAll("<{red}警告{default}> 生还者推进过远, 尸潮强度达到 {green}100%%{default}!");
