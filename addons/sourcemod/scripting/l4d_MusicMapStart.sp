...
public void OnPluginStart()
{
	...
	g_hCvarPlay = CreateConVar(					"l4d_music_mapstart_play_roundstart",		"0",			"Play the music to everyone on round starts. (1 - Yes, 0 - No)", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	g_hCvarShowMenu = CreateConVar(				"l4d_music_mapstart_showmenu_roundstart",	"0",			"Show !music menu on round start? (1 - Yes, 0 - No)", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_hCvarPlay2 = CreateConVar(				"l4d_music_mapstart_play_joinserver",		"0",			"Play the music to client after player joins server? (1 - Yes, 0 - No)", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_hCvarDelay2 = CreateConVar(				"l4d_music_mapstart_delay_joinserver",		"0",			"Delay (in sec.) playing the music to client after player joins server.", CVAR_FLAGS, true, 0.0);
	...
	g_hCvarDownloadMusicNumber = CreateConVar(	"l4d_music_mapstart_download_number",		"99",			"How many random music files to download from 'data/music_mapstart.txt' each map. [0 - all at once]", CVAR_FLAGS, true, 0.0);
	...
	BuildPath(Path_SM, g_sListPath, sizeof(g_sListPath), "data/music.txt");
...
{
	StopSoundCustom;
    g_hCvarEnable.IntValue = 0;
}

public void OnReadyUpInitiate()
{
    g_hCvarEnable.IntValue = 1;
}
