...
public void OnPluginStart()
{
	...
	sm_reserved_slots = CreateConVar("sm_reserved_slots", "0", "预留玩家位置数量", 0, true, 0.0);
	sm_hide_slots = CreateConVar("sm_hide_slots", "0", "如果设置为 1,则保留位置将隐藏(从最大人数中减去)", 0, true, 0.0, true, 1.0);
	sm_reserve_type = CreateConVar("sm_reserve_type", "0", "保留位置的类型", 0, true, 0.0, true, 2.0);
	sm_reserve_maxadmins = CreateConVar("sm_reserve_maxadmins", "1", "允许进入具有保留类型 2 的服务器的最大管理员数量", 0, true, 0.0);
	sm_reserve_kicktype = CreateConVar("sm_reserve_kicktype", "0", "如何选择要踢的客户端（如果开启）", 0, true, 0.0, true, 2.0);
