...
public Action Cmd_WarpToSurvivor(int iClient, int iArgs)
{
	...
	if (g_fGhostWarpDelay[iClient] >= GetGameTime()) {
		PrintToChat(iClient, "%s 你不能连续传送, 请等待 \x04%.01f\x01 秒。", PLUGIN_TAG_COLOR, g_fGhostWarpDelay[iClient] - GetGameTime());
		return Plugin_Handled;
	}

	if (iArgs < 1) {
		if (!WarpToRandomSurvivor(iClient, g_iLastTargetSurvivor[iClient])) {
			PrintToChat(iClient, "%s 找不到 \x04生还者\x01 !", PLUGIN_TAG_COLOR);
		}
