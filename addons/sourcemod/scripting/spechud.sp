	...
	switch (g_Gamemode)
	{
		...
		case GAMEMODE_VERSUS:
		{
			if (bHybridScoremod)
			{
				...
				FormatEx(info, sizeof(info), "> 分数: %i <%.1f%%>", totalBonus, L4D2Util_IntToPercentFloat(totalBonus, maxTotalBonus));
				DrawPanelText(hSpecHud, info);
	...
	for (int client = 1; client <= MaxClients; ++client)
	{
		...
		if (!IsPlayerAlive(client)) 
		{
			...
			else // Ghost Countdown
			{
				...
				// verygood: Dead (15s)
				FormatEx(info, sizeof(info), "%s: 死亡 (%s)", name, (timeLeft ? buffer : "复活中..."));
...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
	...
	if (!IsFakeClient(tank))
	{
		FormatEx(info, sizeof(info), "网络/lerp: %ims / %.1f", RoundToNearest(GetClientAvgLatency(tank, NetFlow_Both) * 1000.0), LM_GetLerpTime(tank) * 1000.0);
	}
	else
	{
		info = "网络: AI";
		...
		case GAMEMODE_VERSUS:
		{
			...
			if (l4d_tank_percent != null && l4d_witch_percent != null)
			{
				...
				if (iTankCount > 0)
				{
					...
					{
						FormatEx(info, sizeof(info), "坦克: %s", (bStaticTank ? "固定生成" : "无"));
					}
				}
				...
				// current
				if (bDivide) {
					Format(info, sizeof(info), "%s | 路程: %i%%", info, survivorFlow);
				} else {
					FormatEx(info, sizeof(info), "路程: %i%%", survivorFlow);
				}
