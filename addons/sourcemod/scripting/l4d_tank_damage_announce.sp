...
PrintRemainingHealth()
{
	...
	if (IsFakeClient(tankclient)) name = "%s";
	else GetClientName(tankclient, name, sizeof(name));
	CPrintToChatAll("{default}[{green}!{default}] {blue}坦克 {default}({olive}%s{default}) 还剩下 {green}%d {default}点血量", name, g_iLastTankHealth);
}
	...
	if (!bPrintedHealth)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if(g_iWasTank[i] > 0)
			{
				...
				CPrintToChatAll("{default}[{green}!{default}] 对 {blue}坦克 {default}({olive}%s{default}) 造成的 {blue}伤害 {default}", name);
				g_iWasTank[i] = 0;
			}
			else if(g_iWasTankAI > 0) 
				CPrintToChatAll("{default}[{green}!{default}] 对 {blue}坦克 {default}({olive}%s(AI){default}) 造成的 {blue}伤害 {default}");
			g_iWasTankAI = 0;
		}
	}
