...
{
	g_hCvarDuration = 			CreateConVar("l4d_kickloadstuckers_duration", 				"60", " How long before a connected but not ingame player is kicked. (default 150) ", 	FCVAR_NOTIFY, true, 1.0);
...
{
	PrintToChatAll("\x05[管理员]踢出了所有正在加载的玩家");
...
Action CheckClientIngame(Handle timer, any client)
{
	...
	if (!IsClientInGame(client))
	{
		...
		PrintToChatAll("\x05%N 因为加载时间超过 %.0f 秒被踢出", client, g_fCvarDuration);
