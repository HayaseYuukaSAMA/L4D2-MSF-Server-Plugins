...
void PrintTopSkeeters(int client)
{
	...
	{
		if(g_hCvar1v1Separate.BoolValue && Is1v1)
			FormatEx(sBuffer, sizeof(sBuffer), "前五名1shot次数最多的大佬(1v1)");
		else
			FormatEx(sBuffer, sizeof(sBuffer), "前五名1shot次数最多的大佬");
	}
	else
	{
		if(g_hCvar1v1Separate.BoolValue && Is1v1)
			FormatEx(sBuffer, sizeof(sBuffer), "空爆次数前 %d 名的大佬(1v1)", TOP_NUMBER);
		else 
			FormatEx(sBuffer, sizeof(sBuffer), "空爆次数前 %d 名的大佬", TOP_NUMBER);
	}
	...
	if (totalskeets)
	{
		...
		{
			FormatEx(sBuffer, sizeof(sBuffer), "%d 次空爆 - %s", CTopPlayer[i].m_iSkeets, CTopPlayer[i].m_sName);
			panel.DrawItem(sBuffer);
		}
		panel.DrawText("\n ");
		FormatEx(sBuffer, sizeof(sBuffer), "在这个服务器里有 %d 次空爆%s", totalskeets, (g_hCvar1v1Separate.BoolValue && Is1v1) ? " in 1v1." : ".");
		panel.DrawText(sBuffer);
	}
	else
	{
		Format(sBuffer, sizeof(sBuffer), "此服务器上尚无空爆次数统计%s", (g_hCvar1v1Separate.BoolValue && Is1v1) ? "(1v1)" : ".");
	}
