...
public OnPluginStart()
{
	...
	l4d_fyzb_init = CreateConVar("l4d_fyzb_init", "1", " 1 : initial enable , 0: initial disable ", FCVAR_PLUGIN);
	...
	l4d_fyzb_anglediff1 = CreateConVar("l4d_fyzb_anglediff1", "9.0", "wall jump difficult >=0", FCVAR_PLUGIN);
 	...
 	l4d_fyzb_safegravity = CreateConVar("l4d_fyzb_safegravity", "1.0", "fall down gravity for safe [0.3-1.0]", FCVAR_PLUGIN);
	...
	HtEnable = FindConVar("l4d_fyzb_enabled");
	HtMsg = FindConVar("l4d_fyzb_showtime");
 	Reset();
}
public OnMapStart()
{
...
{
	if(GetConVarInt(l4d_fyzb_enabled) == 1)
	{
	    if(timer_handle != INVALID_HANDLE )
	    {
		KillTimer(timer_handle);
