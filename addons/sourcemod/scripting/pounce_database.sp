...
void PrintTopPouncers(int client = 0)
{
	...
	FormatEx(sBuffer, sizeof(sBuffer), "高扑次数前 %d 名的大佬", TOP_NUMBER);
	...
	if (totalspounces)
	{
		...
		{
			FormatEx(sBuffer, sizeof(sBuffer), "%d 次高扑 - %s", CTopPlayer[i].m_iPounces, CTopPlayer[i].m_sName);
			panel.DrawItem(sBuffer);
		}
		panel.DrawText("\n ");
		FormatEx(sBuffer, sizeof(sBuffer), "在这个服务器里一共有 %d 次高扑", totalspounces);
		panel.DrawText(sBuffer);
	}
	else
	{
		FormatEx(sBuffer, sizeof(sBuffer), "此服务器上尚无高扑次数统计");
	}
...
Action Timer_Statistic(Handle timer, int attacker)
{
	...
	if(attacker > 0 && IsClientInGame(attacker))
	{
		...
		if(hChat.BoolValue)
		{
			PrintToChat(attacker, "\x01[\x04SM\x01] \x03你 \x04一共有 \x01%d 次高扑", pounce);
		}
	}
