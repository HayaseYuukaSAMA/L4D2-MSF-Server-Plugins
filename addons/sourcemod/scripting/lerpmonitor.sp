...
void ProcessPlayerLerp(int client, bool load = false, bool team = false) 
{
	...
	if ((FloatCompare(newLerpTime, cVarMinLerp.FloatValue) == -1)  || (FloatCompare(newLerpTime, cVarMaxLerp.FloatValue) == 1)) {
		...
		if (!load) {
			...
			if (cVarBadLerpAction.IntValue == 1) {
				CPrintToChatAllEx(client, "{default}<{olive}Lerp{default}> {teamcolor}%N {default}因为 {teamcolor}%.01f lerp被移至旁观者,请分别在控制台输入cl_interp_ratio 0和cl_interp 0将lerp调整为0后按m加入阵营", client, newLerpTime * 1000);
				CPrintToChatEx(client, client, "{default}<{olive}Lerp{default}> Lerp限制:(最小值: {teamcolor}%.01f{default}, 最大值: {teamcolor}%.01f{default})", cVarMinLerp.FloatValue * 1000, cVarMaxLerp.FloatValue * 1000);
				ChangeClientTeam(client, L4D_TEAM_SPECTATE);
			} else {
