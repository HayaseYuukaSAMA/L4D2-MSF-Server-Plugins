	...
	if (strcmp(classname, "infected") == 0 && IsInfiniteHordeActive()) {
		...
		if (commonTotal >= commonLimit) {
			if (!announcedEventEnd){
				CPrintToChatAll("<{olive}有限尸潮事件{default}> 剩余 {red} 0 只{default}尸潮生成……");
		...
		if (hCvarHordeCheckpointAnnounce.BoolValue && 
			...
			if (remaining != 0) {
				CPrintToChatAll("<{olive}有限尸潮事件{default}> 剩余 {red}%i 只{default}尸潮生成..", remaining);
			}
...
public Action L4D_OnSpawnMob(int &amount)
{
	...
	if (IsInfiniteHordeActive()) {
		if (!announcedInChat) {
			CPrintToChatAll("<{olive}有限尸潮事件{default}> 机关已开启！ 生成了 {olive}%i{default} 只 {blue}有限尸潮{default} ! 冲刺或者守点, 选择权在你们手中!", commonLimit);
